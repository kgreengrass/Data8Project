...
def SA_table_import():
    for index, row in addIDs('SA').iterrows():
        # print(index,row) #Test option
        cursor.execute("INSERT INTO SpartaDB.dbo.Self_Development (candidate_id, week_1, week_2, week_3, week_4, week_5, week_6, week_7, week_8, week_9, week_10) VALUES (?,?,?,?,?,?,?,?,?,?,?)", row['CandidateID'], row['SA_W1'], row['SA_W2'], row['SA_W3'], row['SA_W4'], row['SA_W5'], row['SA_W6'], row['SA_W7'], row['SA_W8'], row['SA_W9'], row['SA_W10'])
